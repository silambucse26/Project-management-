...
create table if not exists public.pm_help_requests (
  id uuid primary key default gen_random_uuid(),
  requester_name text not null,
  role text not null,
  department text not null,
...
alter table public.pm_help_requests enable row level security;

create policy "read messages" on public.pm_messages for select using (true);
create policy "insert messages" on public.pm_messages for insert with check (true);

create policy "read help requests" on public.pm_help_requests for select using (true);
create policy "insert help requests" on public.pm_help_requests for insert with check (true);
create policy "update help requests" on public.pm_help_requests for update using (true) with check (true);
