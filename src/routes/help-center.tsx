...
function mapHelpRequestRow(row: Record<string, unknown>): HelpRequest {
  return {
    id: String(row.id ?? ""),
    category: String(row.category ?? "Technical Issue"),
    title: String(row.title ?? "Untitled request"),
    description: String(row.description ?? ""),
    priority: (row.priority as RequestPriority) ?? "Medium",
    status: (row.status as RequestStatus) ?? "Pending",
    submittedBy: String(row.requester_name ?? "Unknown"),
    submittedById: row.requester_id ? String(row.requester_id) : undefined,
...
async function loadHelpRequestsFromDatabase(): Promise<HelpRequest[]> {
  if (!supabase) return initialHelpRequests;

  const { data, error } = await supabase
    .from("pm_help_requests")
    .select("*")
    .order("created_at", { ascending: false });

  if (error) {
    console.error("Failed to load help requests from Supabase", error);
  ...
  return data.map(mapHelpRequestRow);
}

async function saveHelpRequestToDatabase(request: HelpRequest, role: string) {
  if (!supabase) return;

  const row = {
    id: request.id,
    requester_name: request.submittedBy,
    requester_id: request.submittedById ?? null,
    role,
  ...
  const { error } = await supabase.from("pm_help_requests").insert([row]);

  if (error) {
    console.error("Failed to save help request to Supabase", error);
    throw error;
  }
}

async function updateHelpRequestInDatabase(
  requestId: string,
  updates: Partial<{ status: RequestStatus; assignedTo: string | undefined; updatedAt: string }>,
) {
  if (!supabase) return;
  ...
  if (error) {
    console.error("Failed to update help request in Supabase", error);
    throw error;
  }
}
...
function HelpCenterPage() {
  ...
  useEffect(() => {
    ...
    const loadRequests = async () => {
      const fromDatabase = await loadHelpRequestsFromDatabase();
      if (isMounted) {
        setRequests(fromDatabase);
      }
    };
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "pm_help_requests" },
        async () => {
          const fromDatabase = await loadHelpRequestsFromDatabase();
          if (isMounted) {
            setRequests(fromDatabase);
...
function AdminHelpCenter({
  ...
  function updateRequestStatus(
    ...
    void updateHelpRequestInDatabase(requestId, {
      status,
      updatedAt,
    })
      .then(() => {
      ...
      .catch(() => {
        toast.error("The status update could not be saved to the database.");
      });
  }
  ...
  function updateRequestAssignment(
    ...
    void updateHelpRequestInDatabase(requestId, {
      assignedTo: isUnassigned ? undefined : assignedTo,
      updatedAt,
    })
      .then(() => {
        toast.success(
      ...
      .catch(() => {
        toast.error("The assignment update could not be saved to the database.");
      });
  }
...
function EmployeeHelpCenter({
  ...
  async function handleSubmitRequest() {
    ...
    try {
      await saveHelpRequestToDatabase(newRequest, role);
      setTitle("");
      setDescription("");
      setPriority("Medium");
      toast.success("Request submitted successfully.");
    } catch {
      toast.error("The request was saved locally, but could not be stored in the database.");
    }
  }
